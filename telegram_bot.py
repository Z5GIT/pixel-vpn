...
def _wizard_prompt(step: str, data: dict) -> str:
  ...
  if step == "label":
    return head + " اسم/برچسب کانفیگ رو بفرست:"
  if step == "protocol":
...
# Telegram command list is intentionally limited to the commands needed by the bot.
async def _set_bot_commands():
    commands = [
        {"command": "start", "description": "شروع"},
        {"command": "help", "description": "راهنما"},
        {"command": "free", "description": "دریافت کانفیگ رایگان روزانه"},
        {"command": "post", "description": "ارسال کانفیگ حجمی (مدیر)"},
        {"command": "cancel", "description": "لغو عملیات جاری"},
    ]
    await _call("setMyCommands", commands=commands)
...
async def _send_help(chat_id: int):
    if _is_admin(chat_id):
        text = (
            ...
            "/post 1GB تا /post 100GB — ساخت و ارسال کانفیگ به کانال\n"
...
async def _handle_message(msg: dict):
  ...
  if not _is_admin(chat_id):
    await _send(chat_id, "این دستور برای مدیر فعال است.")
    return
  ...
  if pending and pending.get("action") == "wizard" and text:
    ...
    if step == "port":
      ...
      if p is None or not (MIN_PORT <= p <= MAX_PORT):
        await _send(chat_id, f" عدد پورت نامعتبره. یه عدد بین {MIN_PORT} تا {MAX_PORT} بفرست:", _wizard_skip_kb("port", f"⏭ پیش‌فرض ({DEFAULT_PORT})"))
        return
      data["port"] = p
      pending["step"] = "volume"
    ...
    if step == "volume":
      ...
      if parsed is None:
        await _send(chat_id, " فرمت درست نیست. مثلاً بفرست: <code>10GB</code> یا <code>500MB</code>", _wizard_unlimited_kb("volume"))
        return
    ...
    if step == "speed":
      ...
      if parsed is None:
        await _send(chat_id, " فرمت درست نیست. یه عدد بفرست، مثلاً <code>20</code> (Mbps)", _wizard_unlimited_kb("speed"))
        return
    ...
    if step == "iplimit":
      ...
      if n is None:
        await _send(chat_id, " یه عدد صحیح بفرست:", _wizard_unlimited_kb("iplimit"))
        return
      data["ip_limit"] = n
      pending["step"] = "days"
    ...
    if step == "days":
      n = _parse_nonneg_int(text)
      if n is None:
        await _send(chat_id, " یه عدد صحیح بفرست (تعداد روز):", _wizard_unlimited_kb("days"))
        return
      data["expires_days"] = n
...
async def _handle_callback(cb: dict):
  ...
  if data == "newsub":
    ...
    await _edit(chat_id, message_id, " اسم گروه رو بفرست (این اسم فقط برای خودت توی مدیریت گروه‌هاست):", _wizard_cancel_kb())
    return
  ...
  if data.startswith("subdel:"):
    ...
    if not s:
      await _edit(chat_id, message_id, "این گروه دیگه وجود نداره.", _main_menu_kb())
      return
    await _edit(chat_id, message_id, f" از حذف گروه «{s.get('name')}» مطمئنی؟ لینک ساب حرفه‌ای‌اش دیگه کار نمی‌کنه (کانفیگ‌ها حذف نمی‌شن، فقط از گروه خارج می‌شن).", _confirm_subdel_kb(sid))
    return
  ...
  if data.startswith("cfgnewgroup:"):
    ...
    await _edit(chat_id, message_id, " اسم گروه جدید رو بفرست؛ بعد از ساخته شدن، همین کانفیگ خودکار داخلش قرار می‌گیره:", _wizard_cancel_kb())
    return
  ...
  if data.startswith("link:"):
    ...
    if sid and sid in SUBS:
      ...
      msg += "\n\nℹ این کانفیگ توی هیچ گروهی نیست. برای گرفتن لینک ساب حرفه‌ای، از دکمه‌ی « گروه ساب» توی صفحه‌ی کانفیگ استفاده کن."
  ...
  if data.startswith("del:"):
    ...
    if not l:
      await _edit(chat_id, message_id, "این کانفیگ دیگه وجود نداره.", _main_menu_kb())
      return
    await _edit(chat_id, message_id, f" از حذف «{l.get('label')}» مطمئنی؟ این عمل برگشت‌ناپذیره.", _confirm_delete_kb(uid))
    return
...
# ── Lifecycle ────────────────────────────────────────────────────────────────
async def start_bot():
  global _client, _poll_task, _running, API_BASE
  API_BASE = f"https://api.telegram.org/bot{BOT_TOKEN}"
  if not BOT_TOKEN:
